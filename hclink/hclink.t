    ...
    if(_sector = SEC_CODE) do
        ie(_curr = hclink.LNK_CODE) do
            ...
            ie(ref_size = 2) do
                ...
                _pos := _pos + ref_size;
            end else error("Reference size unknown");
...
step_library() do
    if(_curr \= tokens.TK_END) do
        emit_tok(_curr, _curr_text, _curr_len);
        ie(_curr = tokens.TK_MARKER_FILE) do
            ie(_in_count = 0) do
            end else do
                io.writeln("[ OK ]");
            end
            ...
            io.writes(" - Add file ");
            io.writes(_curr_text);
        end else if(_curr = tokens.TK_MARKER_LINE) do
            io.writes(".");
        end
    end
end
